...
public class EsenzioneAcceleratore {
	LogUtil log = new LogUtil(this.getClass(), LogUtil.APPLICATION_CODE_ESENRED, null);

	protected Long id;
	
	private static Set<LuogoNascita> listComuni;
...
//	}
	...
	public EsenzioneAcceleratore(EsenredTEsenzioniReddito esenred, Cittadino beneficiario, Cittadino dichiarante,Cittadino titolare) {
		
		String methodName = "EsenzioneAcceleratore";
		log.info(methodName, "BEGIN");
		
		listComuni = new TreeSet<LuogoNascita>();
		
		this.rapporto_familiare = new TitoloDichiaranteAcceleratore(esenred.getEsenredCTitoloDichiarante());
		this.id = ((esenred.getIdEsenzione()==null) ? new Long(0):esenred.getIdEsenzione());
...
//			this.storico = "Esenzione in corso";
		...
		String attestato = esenred.getAttestato() == null ? "0" : esenred.getAttestato();
		 if (esenred.getNumProtocolloSogei() != null) {
			 if (esenred.getNumProtocolloSogei()!=0) { 
			 if (this.id==0 && esenred.getNumProtocolloSogei().toString().startsWith("1")) {
				 this.protocollo = esenred.getNumProtocolloSogei() != null ? "0" + esenred.getNumProtocolloSogei().toString()
							: null; 
			 }
			 else {
				 this.protocollo = esenred.getNumProtocolloSogei() != null ? esenred.getNumProtocolloSogei().toString()
							: null;  
			 }
			 }
			 else 
		...
		this.stato = new ParametroStatoEsenzione(esenred.getCodStato(),
				StatoEsenzione.getDescrizione(esenred.getCodStato()));
		...
		if (dichiarante.getIdAura()!=null && !dichiarante.getIdAura().equalsIgnoreCase("0")) {
			...
			this.creato_da.luogo_nascita = getLuogoNascita(dichiarante.getCodComuneNascita(),
					dichiarante.getComuneNascita());
		}
	}
		if (beneficiario.getIdAura()!=null && !beneficiario.getIdAura().equalsIgnoreCase("0")) {
			...
			this.creato_per.luogo_nascita = getLuogoNascita(beneficiario.getCodComuneNascita(),
					beneficiario.getComuneNascita());
		}
		if (titolare !=null) {
			...
			this.titolare.luogo_nascita = getLuogoNascita(titolare.getCodComuneNascita(),
					titolare.getComuneNascita());
			}
			else {
				this.creato_per.codice_fiscale = beneficiario.getCodFiscale();
				this.creato_per.cognome = beneficiario.getCognome();
...
//				this.tipoesenzione = "Autocertificata Esenred";
	...
	public LuogoNascita getLuogoNascita(String codComuneNascita, String comuneNascita) {
		try {
		for(LuogoNascita luogo : this.listComuni) {
			
				if (luogo.getCodice().equals(codComuneNascita))
					return luogo;
			
		}
		LuogoNascita tmp = new LuogoNascita(codComuneNascita, comuneNascita);
		this.listComuni.add(tmp);
	...
	public LuogoNascita getLuogoNascita(Cittadino c) {
		for(LuogoNascita luogo : this.listComuni) {
			try {
				if(luogo.getCodice().equals(c.getCodComuneNascita())) return luogo;
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
		LuogoNascita tmp = new LuogoNascita(c);
		this.listComuni.add(tmp);
		return tmp;
	}
	...
	public String toString() {
		return "EsenzioneAcceleratore [id=" + id + ", protocollo=" + protocollo + ", data_richiesta="
				+ data_richiesta + ", data_inizio_validita=" + data_inizio_validita + ", data_scadenza=" + data_scadenza
				+ ", data_revoca=" + data_revoca + ", motivazione_revoca=" + motivazione_revoca + ", revocabile="
				+ revocabile + ", stampabile=" + stampabile + ",  familiare=" + familiare + ", stato=" + stato.toString() + ", codice_esenzione="
				+ codice_esenzione.toString() + ", creato_da=" + creato_da.toString() + ", creato_per=" + creato_per.toString() + ", rapporto_familiare="
				+ rapporto_familiare.toString() + ", codAura=" + codAura + ", messaggio=" + messaggio + ",  titolare=" + titolare.toString() + "]";
	}
}